...
- 已完成关卡收束与底部 GAL 对话重构：`docs/dialogue-story.md` 统一为带中文说话者、中文台词和头像键的单一运行时来源，驾驶员变体也纳入中文检查；`prototype/fb.html` 的关键剧情改为底部双人对话队列，左侧显示当前驾驶员回应、右侧显示对话对象台词，缺少正式美术时使用系统/emoji 占位，点击、空格或 Enter 逐句推进并暂停战斗。第二关已移除阵营选择、分支 Boss 与旧结局出口，调查完成后经黑匣与锈蚀插章唯一进入第 09 隔离关口。第五关移除旧全屏通讯选择层；玛拉对白推进完成后，三项中文行动按钮出现在底部对话框下方，路线对白不会因按钮冒泡被跳过。终局保护目标仍受敌方攻击，玩家投射物仅命中任务节点。验证：`tools/run_checks.ps1` 通过语法、状态/计时器、中文对白数据、关卡收束与 19 个素材检查；本轮独立 HTTP 服务启动被环境策略拦截，尚未完成浏览器端到端通关验收。

- 已完成 Gate 09 通讯剧场纵切：`prototype/fb.html` 新增 `CHAPTER_FIVE`（三波、180 帧间隔、210 避难所完整度）与可重置的 `gate09` 状态；无人机/锈蚀混编波次会攻击避难所，失败重试保留已选驾驶员。`#story-choice-screen` 位于改装层之外，使用键盘可达的文字、形状标记和代价按钮，并在 320px 下改为单列。玛拉呼叫后仅由 `chooseGateRoute()` 写入 `route`，再进入对应终局：封门线保护医疗补给并摧毁三处爆破武器节点；冲关线保护撤离船并解除三组牵引锁；深岩线保护过滤舱、回收核心并上传黑匣。三条线分别映射 `empire`、`rebel`、`deep_rock` 对话与结果卡；`resetRun()` 清除覆盖层、路线与 Gate 09 状态。验证：`powershell -NoProfile -ExecutionPolicy Bypass -File tools\\run_checks.ps1` 通过（脚本语法、叙事契约、对白解析、第一章开场、19 个素材）。
- B 区调查闭环现保留护航、三次扫描、四波敌机、玩家投射物、素材路径和 B 区重试；移除了 B 区阵营抉择及其 Boss 流程。四波、三次证据和敌方清场完成后，原型进入 `INTERLUDE`，显示驾驶员对应的黑匣收束、`BLACKBOX_RECOVERED` 转场（货单、生命读数、注销名单与第 09 关口塌方警报），再按锈蚀风险与 Bob 未授权位置包的顺序播放插章。3.6 秒后最小 Gate 09 占位入口进入 `CH05_PLAY`；路线仍为 `null`，未在 B 区或插章中选择。验证：状态契约先失败并列出五个缺失状态；完成后 `tools/run_checks.ps1` 通过（含脚本语法、叙事状态、对白与 19 个第一章素材检查）。
- 清理 B 区过期分支契约：已删除旧的 B 区选择与 Boss 对话记录、对应必需 ID、以及已无消费者的净化舰/冲关舰与选择卡素材预加载；保留护航船、扫描特效和普通敌机素材。`tools/run_checks.ps1` 现额外验证 `route` 初始化与重置、旧选择 API 缺席、黑匣转场、插章到 Gate 09 的调用及 B 区清场出口。
- 黑匣与插章转场现使用受 `narrativeRun` 保护的 `narrativeTimers`。`resetRun()` 会递增运行代号并取消全部叙事定时器；已排队的黑匣回调或插章回调无法在重置后启动 Gate 09。检查脚本恢复运行时 Markdown 对话解析兼容性测试，并按函数体验证叙事出口和重置契约。
...
- 修复 Gate 09 终局碰撞顺序：保护对象仍承受敌机和敌方投射物伤害，但不再吞掉玩家对武器节点、牵引锁或黑匣核心的普通子弹，确保未获得穿透改装的重试局也可完成三条路线。
- 已完成叙事纵切的自动验证：`tools/run_checks.ps1` 通过原型语法、状态/计时器契约、对白 ID、对白运行时解析兼容性、第一章开场与 19 个运行时素材检查；另以 Node 断言确认三条 Gate 09 路线、重试基线和保护对象不阻挡玩家任务射击。未完成隔离分支的浏览器交互回归：环境策略阻止后台启动该 worktree 的 PowerShell HTTP 服务，`localhost:8080` 已由主工作区旧服务占用，不能据此冒充新分支结果。
- 已完成功能分支首屏浏览器实测：使用独立 `http://127.0.0.1:4173/prototype/fb.html` 服务加载 `codex/narrative-vertical-slice`，选机、部署、第一章“A 区小行星带”开场任务和通讯台词均实际呈现；浏览器控制台未发现 warning 或 error。Gate 09 及三条终局尚未完成端到端手动通关，因此继续以自动契约验证为准，不将其记录为已试玩完成。
