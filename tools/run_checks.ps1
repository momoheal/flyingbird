...
Invoke-Native { node -e "const fs=require('fs'); const html=fs.readFileSync('prototype/fb.html','utf8'); const states=['PROLOGUE','INTERLUDE','CH05_PLAY','CH05_CHOICE','ENDING_PLAY','ENDING_RESULT']; const missing=states.filter(state=>!html.includes(state)); if(missing.length) throw new Error('Missing narrative states: '+missing.join(', ')); const routes=['seal_gate','break_blockade','deep_rock']; const missingRoutes=routes.filter(route=>!html.includes(route)); if(missingRoutes.length) throw new Error('Missing Gate 09 routes: '+missingRoutes.join(', ')); const legacy=['chooseBranch','startChapterTwoBoss','PurifierBoss','RunnerBoss']; const stale=legacy.filter(token=>html.includes(token)); if(stale.length) throw new Error('Legacy B-sector branch flow remains: '+stale.join(', ')); console.log('narrative state contract ok');" }
